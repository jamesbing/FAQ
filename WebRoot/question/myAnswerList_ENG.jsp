...
<div id="mainContentPos2">
	<div id="mainContent2">
		...
		<div id="breadroadbox">Modify my answers</div>
		<c:if test="${personInfo.isExpert() }">
			<table width=850>
				...
				<tr class='bg2'>
					...
					<td style="font-size: 15;">Reply Content</td>
					<td style="font-size: 15;">Operate</td>
				...
				for(int i = 0; i < lstReply.size(); i++){
					...
					out.println("<td>" +lstReply.get(i).getQuestion().getTitle()+
					 	"</a></td><td>"+"<a href='/FAQ/reply.do?action=edit_ENG&id="
				...
				%>
				<td>
				<input type="button" class="bigBtn" value="Modify"
					onclick="window.location.href='/FAQ/reply.do?action=edit_ENG&id=<%out.print(lstReply.get(i).getId());%>'" />
				<input type="button" class="bigBtn" value="Delete"
					onclick="window.location.href='/FAQ/reply.do?action=deleteMyAnswer_ENG&id=<%out.print(lstReply.get(i).getId());%>'" />
				</td>
				</tr>
				<%
				}
