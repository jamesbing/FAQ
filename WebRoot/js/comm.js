...
function getNowTime() {
	...
	// var weekday = new Array(commDateWeek7, commDateWeek1, commDateWeek2,
	// commDateWeek3, commDateWeek4, commDateWeek5, commDateWeek6);
...
function initBuildingInfo() {
	...
	doc = doc
			+ "	<div id='buildingInfoMsg' style='margin-top: 35px; width: 290px;'>";
	doc = doc + "此功能还在建设中，尚不能使用。";
	doc = doc + "</div>";
	doc = doc + "<div style='margin-top: 20px;'>";
	doc = doc
			+ "<input type='button' onclick='closeBuildingInfo();' value='关闭' class='bigBtn' />";
	doc = doc + "</div>";
	doc = doc + "</div>";
	return doc;
}
...
function showCommDialog01(msgString) {
	...
	htmlStr = htmlStr
			+ "<div id='commDialog01Msg' style='margin-top: 35px; width: 290px;'>";
	htmlStr = htmlStr + "</div>";
...
$(function() {
	...
	// $("#userNameBox").find("span").append("<a id='roleIDChange'
	// style='padding-top:5px;padding-left:4px'>" + changeRoleMsg + "</a>");
	// $("#roleIDChange").css({'cursor':'pointer'});
	// $("#roleIDChange").mouseover(function () {
	// var doc = "<div id='changeRoleInfo'
	// style='top:"+(parseInt($("#roleIDChange").offset().top,10)+25)+"px;
	// left:"+(parseInt($("#roleIDChange").offset().left,10)+17)+"px;
	// position:absolute; border-width:1px; border-color:#000000;
	// border-style:solid; padding:1px 5px; font-size:12px;
	// background-color:#FFFFCC;'>"+changeRoleMsg+"</div>";
	// $("body").append(doc);
	// });
	...
	// var doc = "<div id='changeRoleInfo'
	// style='top:"+(parseInt($("#userNameBox2").offset().top,10)+25)+"px;
	// left:"+(parseInt($("#userNameBox2").offset().left,10)+17)+"px;
	// position:absolute; border-width:1px; border-color:#000000;
	// border-style:solid; padding:1px 5px; font-size:12px;
	// background-color:#FFFFCC;'>"+changeRoleMsg+"</div>";
	// $("body").append(doc);
...
function addCurrentTagClass(titleId) {
	$("#" + titleId).addClass("currentTag");
};
...
function showCNUG05Window() {
	...
	htmlStr = htmlStr
			+ "<div id='cnug05DialogMsg' style='margin-top: 35px; width: 290px;'>";
	htmlStr = htmlStr + "</div>";
	htmlStr = htmlStr + "<div style='margin-top: 20px;'>";
	htmlStr = htmlStr
			+ "<input type='button' onclick='closeCnug05Dialog();' value='确定' class='bigBtn' />";
	htmlStr = htmlStr + "</div>";
	htmlStr = htmlStr + "</div>";
	$("body").append(htmlStr);
			...
			.ajax({
				...
				success : function(data, textStatus) {
					...
					if ("0" != checkInfo.errId) {
						...
						var retObj = window
								.open("cnug0502Action.do", "_blank",
										"toolbar=no,directories=no,menubar=no,scrollbars=yes,width=850px,height=650px");
					}
				},
